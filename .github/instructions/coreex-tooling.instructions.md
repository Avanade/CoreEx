...
>
> Do **not** fully-qualify inside the `.g.cs` and do **not** reintroduce a placeholder/marker type just to keep a namespace non-empty — add the `global using` alongside the real code that needs it. (The `AssemblyMarker` types in **Application** and **Infrastructure** are **not** for this — they exist solely as stable `AddDynamicServicesUsing(...)` assembly anchors; see `coreex-host-setup.instructions.md`.)
