...
- **Generate `*.Test.Unit/Clients/{ExternalDomain}/{External}HttpClientTests.cs`** whenever the adapter includes a typed HTTP client — cover success (2xx), server error (5xx), and business error (422/ProblemDetails) per endpoint; skip for EF-only replication adapters (no HTTP, no client tests needed)
