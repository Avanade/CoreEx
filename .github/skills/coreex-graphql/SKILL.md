...
- Use `args.GetIdentifier<TId>(name = "id")` inside an `AddGet<T>` resolver to read/convert the identifier argument — it throws an `ArgumentException` (mapped to `ARGUMENT_ERROR`) if missing/empty/wrong-typed. Do not reach for `args.TryGetValue(...)` directly — `GraphQLLiteArgs` is not a dictionary; use `args.Arguments.TryGetValue(...)` only for non-identifier arguments.
