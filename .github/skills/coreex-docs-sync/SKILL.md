...
- **A skill set change requires an AI client restart to take effect.** Files on disk update immediately, but the client's skill catalog is typically loaded once at startup — a new chat/session in the same running app may still show the stale list. If the diff added, removed, or renamed a skill folder, tell the user to fully restart the AI client (not just open a new session) before relying on the updated catalog.
