...
{
    /// <summary>
    /// Adds <b>singleton</b> <see cref="PostgresOutboxRelayHostedService"/> keyed service(s) (as per <paramref name="servicesCount"/>) that will be executed as a hosted service (in the background).
    /// </summary>
    ...
    /// <param name="serviceKeyPrefix">The keyed singleton and health check key prefix; defaults to '<c>postgres-outbox-relay-</c>'.</param>
    /// <param name="configure">An optional action to configure each <see cref="PostgresOutboxRelayHostedService"/> instance.</param>
    ...
    public static IHostApplicationBuilder AddPostgresOutboxRelayHostedService(this IHostApplicationBuilder builder, int? servicesCount = null, string serviceKeyPrefix = "postgres-outbox-relay-", Action<IServiceProvider, PostgresOutboxRelayHostedService>? configure = null)
    {
        ...
        for (int i = 0; i < servicesCount; i++)
            builder.Services.ThrowIfNull().AddHostedService<PostgresOutboxRelayHostedService>($"{serviceKeyPrefix}{i:00}", configure);
