...
///         .AddNullField(nameof(Employee.Termination), nameof(EfModel.Employee.TerminationDate), c =&gt; c.Default(new QueryStatement($"{nameof(EfModel.Employee.TerminationDate)} is null"))));
/// </code></para></remarks>
...
public sealed class QueryFilterParser(QueryArgsConfig owner)
{
    ...
    /// </summary>
    /// <remarks>This will be automatically applied to all subsequent field additions; for example <see cref="AddField{T}(string, string?, Action{QueryFilterParseableFieldConfig{T}}?)"/>.</remarks>
    ...
    public QueryFilterParser WithDefaultModelPrefix(string? modelPrefix = null)
    {
    ...
    /// <summary>
    /// Adds a <see cref="QueryFilterEnumFieldConfig{T}"/> to the parser for the specified <paramref name="field"/> as-is.
    /// </summary>
    ...
    /// <returns>The <see cref="QueryFilterParser"/> to support fluent-style method-chaining.</returns>
    public QueryFilterParser AddField<T>(string field, Action<QueryFilterEnumFieldConfig<T>>? configure = null) where T : notnull, Enum => AddField(field, null, configure);

    /// <summary>
    /// Adds a <see cref="QueryFilterEnumFieldConfig{T}"/> to the parser using the specified <paramref name="field"/> and <paramref name="model"/> (overrides the <see cref="QueryFilterTokenKind.Field"/> <paramref name="field"/>).
    /// </summary>
    ...
    /// <summary>
    /// Adds a <see cref="QueryFilterParseableFieldConfig{T}"/> to the parser for the specified <paramref name="field"/> as-is.
    /// </summary>
    /// <param name="field">The field name used in the query filter specified with the correct casing.</param>
    /// <param name="configure">The optional action enabling further field configuration.</param>
    /// <returns>The <see cref="QueryFilterParser"/> to support fluent-style method-chaining.</returns>
    public QueryFilterParser AddField<T>(string field, Action<QueryFilterParseableFieldConfig<T>>? configure = null) where T : notnull, IParsable<T> => AddField(field, null, configure);

    /// <summary>
    /// Adds a <see cref="QueryFilterParseableFieldConfig{T}"/> to the parser using the specified <paramref name="field"/> and <paramref name="model"/> (overrides the <see cref="QueryFilterTokenKind.Field"/> <paramref name="field"/>).
    /// </summary>
    /// <param name="field">The field name used in the query filter.</param>
    /// <param name="model">The model name (defaults to <paramref name="field"/>).</param>
    /// <param name="configure">The optional action to perform further field configuration.</param>
    ...
    /// <param name="configure">The optional action to perform further field configuration.</param>
    /// <returns>The <see cref="QueryFilterParser"/> to support fluent-style method-chaining.</returns>
    public QueryFilterParser AddReferenceDataField<TRef>(string field, Action<QueryFilterReferenceDataFieldConfig<TRef>>? configure = null) where TRef : IReferenceData, new() => AddReferenceDataField<TRef>(field, null, configure);
    ...
    /// <summary>
    /// Tries and gets the specified <paramref name="field"/> <paramref name="config"/>.
    /// </summary>
    /// <param name="field">The field name used in the query filter.</param>
    ...
    {
        var result = new QueryFilterParserResult(Owner);
        ...
        try
        {
            ...
            foreach (var expression in GetExpressions(filter))
            {
                if (expression is IQueryFilterFieldStatementExpression fse)
                {
                    ...
                    if (fse.FieldConfig.ResultWriter is not null && fse.FieldConfig.ResultWriter.Invoke(fse, result.Writer))
                        continue;
                }

                expression.WriteAsDynamicLinq(result.Writer);
            }
            ...
            foreach (var statement in _fields.Where(x => x.Value.DefaultStatement is not null && !result.Fields.Contains(x.Key)).Select(x => x.Value.DefaultStatement!))
            {
                var stmt = statement();
                if (stmt is not null)
                    result.Writer.AppendStatement(stmt);
            }
        ...
        {
            qfpex.WithExtension("schema", owner.ToJsonSchema());
            result.Error = qfpex;
        }
        ...
        if (!string.IsNullOrEmpty(filter) && !string.IsNullOrWhiteSpace(filter))
        {
            ...
            foreach (var t in GetRawTokens(filter))
            {
                ...
                else
                {
                    ...
                    {
                        if (!canOpenParen)
                            throw new QueryFilterParserException($"There is a '{t.GetRawToken(filter)}' positioning that is syntactically incorrect.");
                    ...
                    else if (t.Kind == QueryFilterTokenKind.CloseParenthesis)
                    {
                        ...
                        if (parenDepth == 0)
                            throw new QueryFilterParserException($"There is a closing '{t.GetRawToken(filter)}' that has no matching opening '('.");
    ...
    public override string ToString()
    {
        ...
        if (HasFields)
        {
            sb.Append("Filter field(s) are as follows:");
            ...
            sb.Append("Filter statement is not currently supported.");
    ...
    public JsonElement ToJsonSchema()
    {
        ...
        foreach (var field in _fields)
        {
            dict.Add(field.Key.ToLowerInvariant(), field.Value.ToSchemaDictionary());
        }
