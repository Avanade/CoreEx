...
public readonly struct QueryFilterToken(QueryFilterTokenKind kind, int index, int length)
{
    ...
    public readonly ReadOnlySpan<char> GetRawToken(string filter) => filter.ThrowIfNull().AsSpan(Index, Length);
    ...
    public readonly object GetConvertedValue(QueryFilterToken operation, QueryFilterToken field, IQueryFilterFieldConfig config, string filter)
    {
        ...
        try
        {
            return config.ConvertToValue(operation, this, filter) ?? throw new InvalidOperationException($"Field '{field.GetRawToken(filter)}' has a value '{GetValueToken(filter)}' which has been converted to null.");
        }
        ...
        {
            throw new QueryFilterParserException($"Field '{field.GetRawToken(filter)}' with value '{GetValueToken(filter)}' is invalid: {vex.Message}");
        }
        catch (FormatException fex)
        {
            throw new QueryFilterParserException($"Field '{field.GetRawToken(filter)}' has a value '{GetValueToken(filter)}' that is not a valid {config.Type.Name}: {fex.Message}");
        }
        catch (Exception)
        {
            throw new QueryFilterParserException($"Field '{field.GetRawToken(filter)}' has a value '{GetValueToken(filter)}' that is not a valid {config.Type.Name}.");
        }
    }
    ...
    /// </summary>
    /// <param name="kind">The overriding <see cref="QueryFilterTokenKind"/>.</param>
