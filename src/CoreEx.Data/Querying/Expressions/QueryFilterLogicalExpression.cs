...
public class QueryFilterLogicalExpression(QueryFilterParser parser, string filter, QueryFilterToken logical) : QueryFilterExpressionBase(parser, filter, logical)
{
    ...
    public override bool CanAddToken(QueryFilterToken token)
    {
        ...
        return _isComplete
            ? false
            : throw new QueryFilterParserException($"A '{_not.GetRawToken(Filter)}' expects an opening '(' to start an expression versus a syntactically incorrect '{token.GetValueToken(Filter)}' token.");
    }
    ...
    public override void WriteAsDynamicLinq(QueryFilterParserWriter writer)
    {
        if (_logical.Kind != QueryFilterTokenKind.Unspecified)
            writer.AppendWithSpacing(_logical.ToLinq(Filter));

        if (_not.Kind != QueryFilterTokenKind.Unspecified)
            writer.AppendWithSpacing(_not.ToLinq(Filter));
    }
}