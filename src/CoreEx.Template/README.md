...
|---|---|---|
| `coreex-ai` | CoreEx AI assets | `.github/` instructions, prompts, skills, and agents + `.claude/` commands — run at repo root; supports `--app-folder` for monorepos |
...
| `coreex-domain` | CoreEx Domain layer | `src/[name].Domain/` DDD project — add-on, run after `coreex` when domain complexity warrants it |
