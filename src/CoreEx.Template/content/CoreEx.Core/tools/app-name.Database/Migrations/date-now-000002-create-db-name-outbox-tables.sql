-- Create table: [domain-name].[Outbox] and [domain-name].[OutboxLease]

BEGIN TRANSACTION

CREATE TABLE [domain-name].[Outbox] (
  ...
  INDEX [IX_domain-name_Outbox_PartitionOrder] ([TenantId], [PartitionId], [OutboxId]) INCLUDE ([Status], [AvailableUtc], [LeaseUntilUtc], [Destination], [Attempts]),
  INDEX [IX_domain-name_Outbox_WorkerPull] ([TenantId], [PartitionId], [Status]) INCLUDE ([OutboxId], [AvailableUtc]),
  INDEX [IX_domain-name_Outbox_CleanUp] ([OutboxId]) INCLUDE ([DequeuedUtc]) WHERE [Status] = 2
);

CREATE TABLE [domain-name].[OutboxLease] (
  [TenantId] NVARCHAR(255) NOT NULL,      -- '(none)' indicates no tenancy.
  [PartitionId] INT NOT NULL,             -- Partition number; computed in application from partition-key.
  [LeaseId] UNIQUEIDENTIFIER NULL,        -- Unique identifier of the lessee.
  [LeaseUntilUtc] DATETIME2 NULL          -- Leased until UTC; after which assume released due to possible application crash.

  CONSTRAINT PK_domain-name_OutboxLease PRIMARY KEY (TenantId, PartitionId)
);
