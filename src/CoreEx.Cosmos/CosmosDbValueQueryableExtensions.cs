...
namespace System.Linq
{
    ...
    public static class CosmosDbValueQueryableExtensions
    {
        ...
        public static IQueryable<CosmosDbValue<T>> WhereType<T>(this IQueryable<CosmosDbValue<T>> query, string typeName) where T : class, IIdentifier, new() => query.Where("type = @0", typeName ?? throw new ArgumentNullException(nameof(typeName)));
        ...
        /// <returns>The query.</returns>
        public static IQueryable<CosmosDbValue<T>> WhereType<T>(this IQueryable<CosmosDbValue<T>> query, Type type) where T : class, IIdentifier, new() => query.WhereType((type ?? throw new ArgumentNullException(nameof(type))).Name);
    }
}