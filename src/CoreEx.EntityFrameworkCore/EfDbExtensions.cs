...
namespace CoreEx.EntityFrameworkCore
{
    ...
    public static class EfDbExtensions
    {
        ...
        public static Task<T?> GetAsync<T, TModel>(this IEfDb efDb, EfDbArgs args, params object?[] keys) where T : class, IEntityKey, new() where TModel : class, new() 
            => efDb.GetAsync<T, TModel>(args, CompositeKey.Create(keys), CancellationToken.None);
        ...
        /// <param name="keys">The key values.</param>
        /// <returns>The entity value where found; otherwise, <c>null</c>.</returns>
        public static Task<T?> GetAsync<T, TModel>(this IEfDb efDb, params object?[] keys) where T : class, IEntityKey, new() where TModel : class, new() 
            => efDb.GetAsync<T, TModel>(new EfDbArgs(efDb.DbArgs), CompositeKey.Create(keys));
        ...
        public static Task DeleteAsync<T, TModel>(this IEfDb efDb, EfDbArgs args, params object?[] keys) where T : class, IEntityKey where TModel : class, new() 
            => efDb.DeleteAsync<T, TModel>(args, CompositeKey.Create(keys), CancellationToken.None);
        ...
        /// <param name="keys">The key values.</param>
        /// <remarks>Where the model implements <see cref="ILogicallyDeleted"/> then this will update the <see cref="ILogicallyDeleted.IsDeleted"/> with <c>true</c> versus perform a physical deletion.</remarks>
        public static Task DeleteAsync<T, TModel>(this IEfDb efDb, params object?[] keys) where T : class, IEntityKey where TModel : class, new() 
            => efDb.DeleteAsync<T, TModel>(new EfDbArgs(efDb.DbArgs), CompositeKey.Create(keys));
