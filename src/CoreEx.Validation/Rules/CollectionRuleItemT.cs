...
namespace CoreEx.Validation.Rules
{
    ...
    public sealed class CollectionRuleItem<TItem> : ICollectionRuleItem
    {
        ...
        /// <summary>
        /// Specifies that the collection is to be checked for duplicates using the item's <see cref="IIdentifier"/> value.
        /// </summary>
        ...
        public CollectionRuleItem<TItem> IdentifierDuplicateCheck(LText? duplicateText = null)
        {
            if (_duplicateCheck)
                throw new InvalidOperationException("A DuplicateCheck, IdentifierDuplicateCheck, or PrimaryKeyDuplicateCheck can only be specified once.");

            if (ItemType.GetInterface(typeof(IIdentifier).Name) == null)
                throw new InvalidOperationException($"A CollectionRuleItem ItemType '{ItemType.Name}' must implement '{nameof(IIdentifier)}' to support {nameof(IdentifierDuplicateCheck)}.");

            _duplicateText = string.IsNullOrEmpty(duplicateText) ? ValidatorStrings.Identifier : duplicateText;
            _duplicateCheck = true;
            _isIdDuplicateCheck = true;
            _ignoreWhereIdIsDefault = false;

            return this;
        }

        /// <summary>
        /// Specifies that the collection is to be checked for duplicates using the item's <see cref="IIdentifier"/> value with an option to <paramref name="ignoreWhereIdIsDefault"/>.
        /// </summary>
        /// <param name="ignoreWhereIdIsDefault">Indicates whether to ignore the <see cref="IIdentifier.Id"/> when has default value for <see cref="Type"/>; useful where the identifier will be generated by the underlying data source on create.</param>
        /// <param name="duplicateText">The duplicate text <see cref="LText"/> to be passed for the error message; defaults to <see cref="ValidatorStrings.Identifier"/>.</param>
        /// <returns>The <see cref="CollectionRuleItem{TItemEntity}"/> instance to support chaining/fluent.</returns>
        public CollectionRuleItem<TItem> IdentifierDuplicateCheck(bool ignoreWhereIdIsDefault, LText? duplicateText = null)
        {
            IdentifierDuplicateCheck(duplicateText);
            _ignoreWhereIdIsDefault = ignoreWhereIdIsDefault;
            return this;
        }
        ...
        private void DuplicateValidation(IPropertyContext context, IEnumerable<TItem>? items)
        {
            ...
            else if (_isIdDuplicateCheck)
            {
                var dict = new Dictionary<object?, object?>();
                foreach (var item in items.Where(x => x != null).Cast<IIdentifier>())
                {
                    if (_ignoreWhereIdIsDefault)
                    {
                        if (item.Id == null)
                            continue;
                    ...
                    {
                        context.CreateErrorMessage(ValidatorStrings.DuplicateValueFormat, _duplicateText!, item.Id);
                        return;
                    }

                    dict.Add(item.Id, null);
                }
            }
            else
            {
