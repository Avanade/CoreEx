...
public class DatabaseWildcard
{
    ...
    public DatabaseWildcard(Wildcard? wildcard = null, char multiWildcard = MultiWildcardCharacter, char singleWildcard = SingleWildcardCharacter, char[]? charactersToEscape = null, string? escapeFormat = null)
    {
        ...
        if (Wildcard.SupportedSelection.HasFlag(WildcardSelection.MultiWildcard) && multiWildcard == char.MinValue)
            throw new ArgumentException("A Wildcard that supports MultiWildcard must also define the database MultiWildcard character.");

        if (Wildcard.SupportedSelection.HasFlag(WildcardSelection.SingleWildcard) && singleWildcard == char.MinValue)
        ...
        if (CharactersToEscape.Count > 0 && string.IsNullOrEmpty(EscapeFormat))
    ...
    public string? Replace(string? text)
    {
        ...
        foreach (var c in wr.Text!)
        {
            ...
            else if (CharactersToEscape is not null && CharactersToEscape.Contains(c))
                sb.Append(string.Format(CultureInfo.InvariantCulture, EscapeFormat, c));
            else
                sb.Append(c);
        }
