...
public sealed class Wildcard
{
    ...
    /// <summary>
    /// Gets or sets the default <see cref="Wildcard"/> settings (defaults to <see cref="WildcardSelection.MultiBasic"/>.
    /// </summary>
    ...
    public Wildcard(WildcardSelection supported, char multiWildcard = MultiWildcardCharacter, char singleWildcard = SingleWildcardCharacter, char[]? charactersNotAllowed = null, WildcardSpaceTreatment spaceTreatment = WildcardSpaceTreatment.None)
    {
        ...
        if (charactersNotAllowed is not null && (charactersNotAllowed.Contains(multiWildcard) || charactersNotAllowed.Contains(singleWildcard)))
        ...
        if (supported.HasFlag(WildcardSelection.MultiWildcard) && multiWildcard == char.MinValue)
            throw new ArgumentException("A Wildcard that supports MultiWildcard must also define the MultiWildcard character.", nameof(multiWildcard));

        if (supported.HasFlag(WildcardSelection.SingleWildcard) && singleWildcard == char.MinValue)
            throw new ArgumentException("A Wildcard that supports SingleWildcard must also define the SingleWildcard character.", nameof(singleWildcard));

        SupportedSelection = supported;
        MultiWildcard = multiWildcard;
        SingleWildcard = singleWildcard;
        CharactersNotAllowed = new ReadOnlyCollection<char>(charactersNotAllowed is not null ? (char[])charactersNotAllowed.Clone() : []);
    ...
    /// </summary>
    public WildcardSelection SupportedSelection { get; }
    ...
    /// </summary>
    public IReadOnlyList<char> CharactersNotAllowed { get; }

    /// <summary>
    /// Gets the <see cref="WildcardSpaceTreatment"/> that defines the treatment of embedded space ' ' characters within the wildcard.
    /// </summary>
    public WildcardSpaceTreatment SpaceTreatment { get; }

    /// <summary>
    /// Parses the wildcard text to ensure validity returning a <see cref="WildcardResult"/>.
    /// </summary>
    /// <param name="text">The wildcard text.</param>
    /// <returns>The corresponding <see cref="WildcardResult"/>.</returns>
    /// <remarks>Note that any leading or trailing spaces are ignored.</remarks>
    public WildcardResult Parse(string? text)
    {
        ...
        if (CharactersNotAllowed is not null && CharactersNotAllowed.Count > 0 && text.IndexOfAny([.. CharactersNotAllowed]) >= 0)
            wr.Selection |= WildcardSelection.InvalidCharacter;

        var hasMulti = SpaceTreatment == WildcardSpaceTreatment.MultiWildcardWhenOthers && SupportedSelection.HasFlag(WildcardSelection.MultiWildcard) && text.Contains(MultiWildcardCharacter, StringComparison.Ordinal);
        var hasTxt = false;
        ...
        wr.HasError = !ValidateSelection(wr.Selection);

        return wr;
    }

    /// <summary>
    /// Validates the <paramref name="selection"/> against what is <see cref="SupportedSelection"/> to ensure validity.
    /// </summary>
    private bool ValidateSelection(WildcardSelection selection)
    {
        if (selection.HasFlag(WildcardSelection.InvalidCharacter) ||
            (selection.HasFlag(WildcardSelection.None) && !SupportedSelection.HasFlag(WildcardSelection.None)) ||
            (selection.HasFlag(WildcardSelection.Equal) && !SupportedSelection.HasFlag(WildcardSelection.Equal)) ||
            (selection.HasFlag(WildcardSelection.Single) && !SupportedSelection.HasFlag(WildcardSelection.Single)) ||
            (selection.HasFlag(WildcardSelection.StartsWith) && !SupportedSelection.HasFlag(WildcardSelection.StartsWith)) ||
            (selection.HasFlag(WildcardSelection.EndsWith) && !SupportedSelection.HasFlag(WildcardSelection.EndsWith)) ||
            (selection.HasFlag(WildcardSelection.Contains) && !SupportedSelection.HasFlag(WildcardSelection.Contains)) ||
            (selection.HasFlag(WildcardSelection.Embedded) && !SupportedSelection.HasFlag(WildcardSelection.Embedded)) ||
            (selection.HasFlag(WildcardSelection.MultiWildcard) && !SupportedSelection.HasFlag(WildcardSelection.MultiWildcard)) ||
            (selection.HasFlag(WildcardSelection.SingleWildcard) && !SupportedSelection.HasFlag(WildcardSelection.SingleWildcard)) ||
            (selection.HasFlag(WildcardSelection.AdjacentWildcards) && !SupportedSelection.HasFlag(WildcardSelection.AdjacentWildcards)))
            return false;
        else
            return true;
    }
