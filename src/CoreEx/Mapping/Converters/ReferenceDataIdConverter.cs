...
namespace CoreEx.Mapping.Converters
{
    ...
    public struct ReferenceDataIdConverter<TRef, TId> : IConverter<TRef, TId> where TRef : IReferenceData<TId>, new() where TId : IComparable<TId>, IEquatable<TId>
    {
        private readonly ValueConverter<TRef, TId> _convertToDestination = new(s => s == null ? default : s.Id);
        private readonly ValueConverter<TId, TRef> _convertToSource = new(d => ReferenceDataOrchestrator.ConvertFromId<TRef, TId>(d));
