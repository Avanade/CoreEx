...
/// </summary>
public readonly struct StringBase64Converter : IConverter<string?, byte[]?>
{
    private static readonly ValueConverter<string?, byte[]?> _convertToDestination = new(s => s is null ? null : Convert.FromBase64String(s));
    private static readonly ValueConverter<byte[]?, string?> _convertToSource = new(d => d is null ? null : Convert.ToBase64String(d));

    /// <summary>
    /// Gets or sets the default (singleton) instance.
    /// </summary>
    public static StringBase64Converter Default { get; set; } = new();

    /// <summary>
    /// Initializes a new instance of the <see cref="StringBase64Converter"/> struct.
    /// </summary>
    public StringBase64Converter() { }
