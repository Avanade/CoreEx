...
{
    private readonly Func<TSource, TDestination> _converter = converter.ThrowIfNull();

    /// <inheritdoc/>
    public TDestination Convert(TSource source) => _converter is null ? throw new InvalidOperationException($"The {nameof(ValueConverter<,>)} has not been initialized with an underlying converter correctly.") : _converter(source);
}