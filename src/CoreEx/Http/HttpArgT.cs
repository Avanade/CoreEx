...
{
    /// <summary>
    /// Represents a typed <see cref="HttpArg"/> argument for an <see cref="HttpRequestMessage"/> where updating <see cref="HttpRequestMessage.RequestUri"/> or <see cref="HttpRequestMessage.Content"/> (body).
    /// </summary>
    /// <typeparam name="T">The <see cref="Value"/> <see cref="Type"/>.</typeparam>
    public class HttpArg<T> : HttpArg
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="HttpArg{Object}"/> class.
        /// </summary>
        /// <param name="name">The argument <see cref="HttpArg.Name"/>.</param>
        /// <param name="value">The argument <see cref="Value"/>.</param>
        /// <param name="argType">The <see cref="HttpArgType"/>.</param>
        public HttpArg(string name, T value, HttpArgType argType = HttpArgType.FromUri) : base(name, argType) => Value = value;
        ...
        /// </summary>
        public override bool IsDefault => Comparer<T>.Default.Compare(Value, default!) == 0;

        /// <summary>
        /// Gets the underlying <see cref="Value"/>.
        /// </summary>
        /// <returns>The underlying value.</returns>
        public override object? GetValue() => Value;

        /// <summary>
        /// Returns a <see cref="Uri.EscapeDataString"/> representation of just the <see cref="Value"/> itself.
        /// </summary>
        /// <returns>The escaped data string.</returns>
        public override string? ToEscapeDataString()
        {
            if (Value == null)
                return null;
        ...
        /// <inheritdoc/>
        public override QueryString AddToQueryString(QueryString queryString, IJsonSerializer? jsonSerializer = null)
        {
            if (IsDefault)
                return queryString;
