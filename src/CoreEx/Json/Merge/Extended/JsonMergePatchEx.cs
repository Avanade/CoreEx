...
{
    /// <summary>
    /// Provides a JSON Merge Patch (<c>application/merge-patch+json</c>) whereby the contents of a JSON document are merged into an existing object value (instance) as per <see href="https://tools.ietf.org/html/rfc7396"/>
    /// using .NET Reflection (see <see cref="TypeReflectorArgs"/>).
    /// </summary>
    ...
    /// <para>Additional logic has been added to the merge patch enabled by <see cref="JsonMergePatchExOptions.DictionaryMergeApproach"/> and <see cref="JsonMergePatchExOptions.EntityKeyCollectionMergeApproach"/>. Note: these capabilities are
    /// unique to <i>CoreEx</i> and not part of the formal specification <see href="https://tools.ietf.org/html/rfc7396"/>.</para></remarks>
    public class JsonMergePatchEx : IJsonMergePatch
    {
        private const string EKCollectionName = "EKColl";
        private readonly TypeReflectorArgs _trArgs;

        /// <summary>
        /// Initializes a new instance of the <see cref="JsonMergePatchEx"/> class.
        /// </summary>
        /// <param name="options">The <see cref="JsonMergePatchExOptions"/>.</param>
        ...
        public JsonMergePatchEx(JsonMergePatchExOptions? options = null)
        {
            Options = options ?? new JsonMergePatchExOptions();

            _trArgs = new TypeReflectorArgs(Options.JsonSerializer)
            {
                AutoPopulateProperties = true,
                NameComparer = Options.PropertyNameComparer,
                TypeBuilder = tr =>
                {
        ...
        /// <summary>
        /// Gets the <see cref="JsonMergePatchExOptions"/>.
        /// </summary>
        public JsonMergePatchExOptions Options { get; }
        ...
        private ICompositeKeyCollection MergeKeyedCollection(ITypeReflector tr, string root, JsonElement json, ICompositeKeyCollection srce, ICompositeKeyCollection? dest, ref bool hasChanged)
        {
            ...
            foreach (var ji in json.EnumerateArray())
            {
                ...
                if (ji.ValueKind != JsonValueKind.Object && ji.ValueKind != JsonValueKind.Null)
                    throw new JsonMergePatchException($"The JSON array item must be an Object where the destination collection supports '{nameof(IEntityKey)}' keys. Path: {path}");
