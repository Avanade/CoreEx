...
{
    /// <summary>
    /// Defines the approach for a collection (see <see cref="ICollection"/>) that implements <see cref="IEntityKeyCollection{T}"/>; being either <see cref="Replace"/> or <see cref="Merge"/>.
    /// </summary>
    ...
    {
        /// <summary>
        /// Indicates that the collection (see <see cref="IEntityKeyCollection{T}"/>) merge will be treated the same as any <see cref="System.Text.Json.JsonValueKind.Array"/> where the result is a replacement (overwrite) operation.
        /// </summary>
        Replace,

        /// <summary>
        /// Indicates that the collection (see <see cref="IEntityKeyCollection{T}"/>) merge will be managed by the checking of the <see cref="IEntityKey.EntityKey"/> for all items. Where the merge JSON does not provide a matching
