...
- 🏷️ **ETag / If-Match concurrency**: `PutAsync`, `PatchAsync`, `PostAsync` and `DeleteAsync` all read the `If-Match` request header (`ro.ETag`) and stamp it onto the request value where it implements `IETag`; for `PutAsync`/`PatchAsync` its absence automatically fails with `412`/`428` before the operation runs. POST and DELETE don't auto-require it (they typically have no prior state to match), but a handler for an operation that *is* conditional on a related resource's state (e.g. adding an item to a booking) can call `ro.WithIfMatchRequired()` as the first line of the handler — it asserts `ro.ETag` immediately and throws a `ConcurrencyException` (→ `428`) if missing, regardless of method or whether the request value implements `IETag`. `PatchAsync` additionally validates the ETag before applying the patch.
