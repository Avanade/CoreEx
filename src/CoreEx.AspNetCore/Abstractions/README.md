...
- 🏷️ **ETag / If-Match concurrency**: `PutAsync`, `PatchAsync`, `PostAsync` and `DeleteAsync` all read the `If-Match` request header (`ro.ETag`) and stamp it onto the request value where it implements `IETag`. Two distinct failure modes, not interchangeable: a **missing** ETag on `PutAsync`/`PatchAsync` (where the request value implements `IETag`) automatically fails with `428 Precondition Required` before the operation runs; a **stale/mismatched** ETag against the persisted value fails separately with `412 Precondition Failed` once the operation actually runs. POST and DELETE don't auto-require the header (they typically have no prior state to match), but a handler for an operation that *is* conditional on a related resource's state (e.g. adding an item to a booking) can call `ro.WithIfMatchRequired()` as the first line of the handler — it asserts `ro.ETag` immediately and throws a `ConcurrencyException` (→ `428` for missing) if the header wasn't supplied, regardless of method or whether the request value implements `IETag`. `PatchAsync` additionally validates the ETag before applying the patch.
- 📃 **Paging header stamping**: When an operation returns an `ICollectionResult` / `ItemsResult<T>`, `WebApiPagingResult` stamps the `X-Paging-Skip`, `X-Paging-Take`, and (when a total count is available) `X-Paging-Total-Count` response headers.
