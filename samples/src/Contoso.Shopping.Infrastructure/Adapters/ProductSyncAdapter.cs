...
public class ProductSyncAdapter(ShoppingEfDb ef) : IProductSyncAdapter
{
    ...
    public async Task<Result> ModifyAsync(Product product, CancellationToken ct = default) => (await _ef.Products.UpsertWithResultAsync(ProductMapper.To.Map(product), ct).ConfigureAwait(false)).AsResult();

    /// <inheritdoc/>
    /// <remarks>Removes the product from the internal event-based replication store.</remarks>
    public async Task<Result> DeleteAsync(string id, CancellationToken ct = default) => (await _ef.Products.DeleteWithResultAsync(id, ct).ConfigureAwait(false)).AsResult();
}