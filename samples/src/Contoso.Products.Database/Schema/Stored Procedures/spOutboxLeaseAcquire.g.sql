CREATE OR ALTER PROCEDURE [Products].[spOutboxLeaseAcquire]
...
BEGIN
  ...
  BEGIN TRY
    ...
    IF NOT EXISTS (
      SELECT 1
      FROM [Products].[OutboxLease] WITH (UPDLOCK, HOLDLOCK)
      WHERE [TenantId] = @EffectiveTenantId AND [PartitionId] = @PartitionId
    )
    BEGIN
      INSERT INTO [Products].[OutboxLease] ([TenantId], [PartitionId])
    ...
    UPDATE ol
      ...
      FROM [Products].[OutboxLease] AS ol WITH (UPDLOCK, ROWLOCK)
