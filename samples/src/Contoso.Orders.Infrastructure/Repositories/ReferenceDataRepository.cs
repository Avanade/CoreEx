...
public class ReferenceDataRepository(OrdersEfDb ef) : IReferenceDataRepository
{
    ...
    public Task<Contracts.OrderStatusCollection> GetAllOrderStatusesAsync()
        => _ef.OrderStatuses.Query().ToMappedItemsAsync<Persistence.OrderStatus, Contracts.OrderStatusCollection, Contracts.OrderStatus>(OrderStatusMapper.From);
}