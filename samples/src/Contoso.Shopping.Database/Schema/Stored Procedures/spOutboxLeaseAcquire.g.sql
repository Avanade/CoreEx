CREATE OR ALTER PROCEDURE [Shopping].[spOutboxLeaseAcquire]
...
BEGIN
  ...
  BEGIN TRY
    ...
    IF NOT EXISTS (
      SELECT 1
      FROM [Shopping].[OutboxLease] WITH (UPDLOCK, HOLDLOCK)
      WHERE [TenantId] = @EffectiveTenantId AND [PartitionId] = @PartitionId
    )
    BEGIN
      INSERT INTO [Shopping].[OutboxLease] ([TenantId], [PartitionId])
