CREATE OR ALTER PROCEDURE [Shopping].[spOutboxBatchClaim]
...
BEGIN
  ...
  DECLARE @RC INT;
  EXEC @RC = [Shopping].[spOutboxLeaseAcquire] @EffectiveTenantId, @PartitionId, @LeaseId, @LeaseSeconds, @LeaseUntilUtc OUTPUT;
  IF (@RC < 0) RETURN -3;
  ...
  BEGIN TRY
    ...
    SELECT @HeadId = MIN(o.OutboxId)
      FROM [Shopping].[Outbox] o WITH (UPDLOCK)
    ...
    SELECT @BlockerId = MIN(o.OutboxId)
      FROM [Shopping].[Outbox] o WITH (READPAST, UPDLOCK)
      WHERE o.[TenantId] = @EffectiveTenantId
        AND o.[PartitionId] = @PartitionId
    ...
    (
      ...
      FROM [Shopping].[Outbox] o WITH (READPAST, UPDLOCK, ROWLOCK)
      WHERE o.[TenantId] = @EffectiveTenantId
        AND o.[PartitionId] = @PartitionId
        AND o.[OutboxId] >= @HeadId
