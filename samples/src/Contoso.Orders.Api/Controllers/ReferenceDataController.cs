...
public class ReferenceDataController(WebApi webApi) : ControllerBase
{
    ...
    public Task<IActionResult> GetOrderStatusesAsync([FromQuery] IEnumerable<string>? codes = default, string? text = default)
        => _webApi.GetAsync(Request, (ro, ct) => ReferenceDataOrchestrator.Current.GetWithFilterAsync<OrderStatus>(codes, text, ro.IsIncludeInactive, ct));
}