...
public class EntryPoint
{
    ...
    public class ReferenceDataProvider(JsonDataReader jdr) : IReferenceDataProvider
    {
        ...
        public Task<IReferenceDataCollection> GetAsync(Type type, CancellationToken cancellationToken = default) => type switch
        {
            ...
            _ when type == typeof(SubCategory) => Task.FromResult((IReferenceDataCollection)jdr.Deserialize<SubCategoryCollection>("Products.$^SubCategory")!),
            _ when type == typeof(UnitOfMeasure) => Task.FromResult((IReferenceDataCollection)jdr.Deserialize<UnitOfMeasureCollection>("Products.$^UnitOfMeasure")!),
            _ when type == typeof(Brand) => Task.FromResult((IReferenceDataCollection)jdr.Deserialize<BrandCollection>("Products.$^Brand")!),
            _ when type == typeof(MovementKind) => Task.FromResult((IReferenceDataCollection)jdr.Deserialize<MovementKindCollection>("Products.$^MovementKind")!),
            _ when type == typeof(MovementStatus) => Task.FromResult((IReferenceDataCollection)jdr.Deserialize<MovementStatusCollection>("Products.$^MovementStatus")!),
