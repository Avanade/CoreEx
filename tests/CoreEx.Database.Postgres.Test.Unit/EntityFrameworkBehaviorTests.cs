...
public class EntityFrameworkBehaviorTests : DatabaseTestBase
{
    ...
    public void Update_Attached_StaleETag_WithoutEfConcurrencyToken_ReturnsConcurrencyError() => Test.ScopedType<ExecutionContext>(test => test.Run(async _ =>
    {
        ...
        var database = ExecutionContext.GetRequiredService<PostgresDatabase>();
        ...
        // Simulate another process changing the row directly; this bumps the database-generated xmin underneath the tracked copy.
        await database.Statement("UPDATE \"test\".\"table\" SET \"text\" = @Text WHERE \"table_id\" = @Id").Param("Text", "ChangedElsewhere").Param("Id", id).NonQueryAsync().ConfigureAwait(false);
    ...
    private sealed class NoConcurrencyTokenDbContext(DbContextOptions<NoConcurrencyTokenDbContext> options, PostgresDatabase database) : DbContext(options), IEfDbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<TestTable>(e =>
            {
                ...
                e.Property(p => p.ETag).HasColumnName("xmin").HasColumnType("xid").ValueGeneratedOnAddOrUpdate().HasConversion(ValueConverterBridge.Create<string?, uint>(BaseDatabase.RowVersionConverter));
