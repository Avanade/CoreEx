...
namespace CoreEx.Test.Framework.Validation.Rules
{
    ...
    public class DictionaryRuleTest
    {
        ...
        public async Task Validate_KeyAndValue()
        {
            ...
            var v1 = await new Dictionary<string, int> { { "k1", 1 }, { "k2", 2 } }.Validate("Dict").Dictionary(item: DictionaryRuleItem.Create(kv, vv)).RunAsync();
            Assert.IsFalse(v1.HasError);

            v1 = await new Dictionary<string, int> { { "k1", 11 }, { "k2x", 2 } }.Validate("Dict").Dictionary(item: DictionaryRuleItem.Create(kv, vv)).RunAsync();
            Assert.IsTrue(v1.HasError);
