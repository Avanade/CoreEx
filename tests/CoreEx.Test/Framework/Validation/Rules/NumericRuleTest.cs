...
namespace CoreEx.Test.Framework.Validation.Rules
{
    ...
    public class NumericRuleTest
    {
        ...
        {
            var v1 = await (123f).Validate().Numeric().ValidateAsync();
            Assert.IsFalse(v1.HasErrors);

            v1 = await (-123f).Validate().Numeric().ValidateAsync();
            ...
            v1 = await (-123f).Validate().Numeric(true).ValidateAsync();
            Assert.IsFalse(v1.HasErrors);

            var v2 = await (123d).Validate().Numeric().ValidateAsync();
            Assert.IsFalse(v2.HasErrors);

            v2 = await (-123d).Validate().Numeric().ValidateAsync();
            ...
            v2 = await (-123d).Validate().Numeric(true).ValidateAsync();
            Assert.IsFalse(v2.HasErrors);
        }
    }
}
