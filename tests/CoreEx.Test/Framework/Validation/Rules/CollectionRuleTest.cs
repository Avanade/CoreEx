...
namespace CoreEx.Test.Framework.Validation.Rules
{
    ...
    public class CollectionRuleTest
    {
        ...
        {
            var v1 = await new List<TestItem?> { new TestItem() }.Validate().Collection().RunAsync();
            Assert.IsFalse(v1.HasError);

            v1 = await new List<TestItem?>() { null }.Validate().Collection().RunAsync();
            Assert.IsTrue(v1.HasError);
            Assert.AreEqual(1, v1.Messages!.Count);
            ...
            v1 = await new List<TestItem?> { null }.Validate().Collection(allowNullItems: true).RunAsync();
            Assert.IsFalse(v1.HasError);
        }
