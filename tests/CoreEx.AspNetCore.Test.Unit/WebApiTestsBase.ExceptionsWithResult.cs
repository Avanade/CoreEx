...
partial class WebApiTestsBase<TWebApi, TResult>
{
    ...
    public void ExceptionWithResult_ProblemHandling(Type type, HttpStatusCode statusCode, string? errorType = null, string? errorCode = null)
    {
        ...
        Test.Type<TWebApi>()
            .Run(async w =>
            {
                ...
                return await w.PostWithResultAsync(Test.CreateHttpRequest(HttpMethod.Post, "test"), (ro, ct) => Task.FromResult(Result.Fail(ex)));
            })
    ...
    [TestCase(typeof(ValidationException), HttpStatusCode.BadRequest, "validation")]
    [TestCase(typeof(InvalidOperationException), HttpStatusCode.InternalServerError)]
    public void ExceptionWithResult_Throw_ProblemHandling(Type type, HttpStatusCode statusCode, string? errorType = null, string? errorCode = null)
    {
        List<string> paths = ["type", "title", "traceid"];
        ...
        Test.Type<TWebApi>()
            ...
            {
                w.ConvertUnhandledExceptionsToProblemDetails = true; // Ensure enabled for unit-testing.
                return await w.PostWithResultAsync(Test.CreateHttpRequest(HttpMethod.Post, "test"), (ro, ct) => throw ex);
            })
            .ToHttpResponseMessageAssertor()
            .Assert(statusCode)
            .AssertContentType(MediaTypeNames.Application.ProblemJson)
